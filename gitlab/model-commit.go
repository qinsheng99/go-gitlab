...
type Actions struct {
	...
	PreviousPath    string `json:"previous_path,omitempty" description:"被移动文件的原始完整路径, 适用于move" required:"false"`
	Content         string `json:"content,omitempty" description:"文件内容，除了 delete、chmod 和 move 外其他操作必填" required:"false"`
	Encoding        string `json:"encoding,omitempty" description:"text 或者 base64。默认值是 text" required:"false"`
	LastCommitId    string `json:"last_commit_id,omitempty" description:"最后一个已知的文件提交 ID。仅适用于update、move 和 delete 操作" required:"false"`
	ExecuteFilemode bool   `json:"execute_filemode,omitempty" description:"适用于 chmod 操作,添加可执行标志" required:"false"`
}
...
type PostCommit struct {
	...
	StartBranch string    `json:"start_branch,omitempty" description:"新创建的分支派生自的分支名称" required:"false"`
